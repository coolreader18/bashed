...
function isValidReservedWordPosition(tk, iterable, words) {
	...
	const twoAgo = iterable.behind(2) || {
		EMPTY: true,
		is: type => type === "EMPTY"
	};
	...
	const startOfCommand =
		last.is("EMPTY") ||
		last.is("SEPARATOR_OP") ||
		last.is("OPEN_PAREN") ||
		last.is("CLOSE_PAREN") ||
		last.is("NEWLINE") ||
		last.is("NEWLINE_LIST") ||
		last.is("DSEMI") ||
		last.value === ";" ||
		last.is("PIPE") ||
		last.is("OR_IF") ||
	...
	const lastIsReservedWord =
		last.value !== "for" &&
		last.value !== "in" &&
		last.value !== "case" &&
		values(words).some(word => last.is(word));

	const thirdInCase =
		twoAgo.value === "case" &&
		tk.is("TOKEN") &&
		tk.value.toLowerCase() === "in";
	const thirdInFor =
		twoAgo.value === "for" &&
		tk.is("TOKEN") &&
...
const reservedWords = (options, mode) => {
	return compose(
		map((tk, idx, iterable) => {
			...
			if (
				isValidReservedWordPosition(tk, iterable, mode.enums.reservedWords) &&
				hasOwnProperty(mode.enums.reservedWords, tk.value)
			) {
