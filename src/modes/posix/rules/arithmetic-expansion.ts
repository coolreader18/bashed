...
function parseArithmeticAST(xp) {
	...
	try {
		...
		throw new SyntaxError(
			`Cannot parse arithmetic expression "${xp.expression}": ${err.message}`
		);
	}
	...
	if (expression === undefined) {
		throw new SyntaxError(
			`Cannot parse arithmetic expression "${xp.expression}": Not an expression`
		);
	}
				...
				token.expansion.map(xp => {
					if (xp.type === "arithmetic_expansion") {
						return Object.assign({}, xp, {
							arithmeticAST: parseArithmeticAST(xp)
						});
					}
					return xp;
				})
