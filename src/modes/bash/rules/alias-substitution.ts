	...
	function* tryExpandToken(token, expandingAliases) {
		...
		if (result === undefined) {
			...
			for (const newToken of preAliasLexer(result)) {
				if (
					newToken.is("WORD") ||
					reservedWords.some(word => newToken.is(word))
				) {
...
export default (options, mode, previousPhases) => {
	...
	const preAliasLexer = apply(null, previousPhases.reverse());
	const visitor = expandAlias(
		preAliasLexer,
		options.resolveAlias,
		values(mode.enums.reservedWords)
	);

	return compose(
		merge,
		map(applyTokenizerVisitor(visitor))
	);
};
